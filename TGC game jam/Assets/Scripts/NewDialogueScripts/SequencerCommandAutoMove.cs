...
/// 控制玩家自动移动到指定目标点，并等待整个异步过程（移动、播放动画、等待）完成后再继续。
/// 语法: AutoMove(targetObjectName, [proximityThreshold])
/// </summary>
public class SequencerCommandAutoMove : SequencerCommand
{
    ...
    private async UniTask DoAutoMoveAsync()
    {
        ...
        await playerMove.AutoMoveToPosition(targetObject.transform.position, proximity);
